...
These records capture the significant technical choices behind Registry, in causal order. Each states the problem it answered, what was chosen, why, and what the choice costs.

They document the system **as it is implemented today**. Where a decision has a cost that is still being paid, the *Consequences* section says so plainly rather than reading as a justification.
...
Two chains run through the set. On the backend, the hexagonal decision makes the reactive and persistence choices tractable, and the authentication decision leads directly to the permission model, which in turn shapes the schema and the retention jobs. On the frontend, the immutable-image requirement forces runtime configuration, which is what makes runtime theming possible at all.
...
New records take the next free number and are added to the table above and to the sidebar. A decision that replaces an earlier one does not edit it — it gets its own record, and the superseded one's status is updated to point at it.
