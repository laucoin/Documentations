...
- **What they do**: search users, change a user's global role, block/unblock accounts, and anonymize (purge) a user on request. They do **not** run events day-to-day unless they also hold a project role.
...
- **What Registry owes them**: complete control of their own event, isolated from every other event, with guard rails that prevent them from accidentally removing the event's last administrator.
...
- **What they do**: full operational management of participants, groups, movements and the optional modules. They **cannot** invite or manage members, and cannot delete the project.
...
- **Registered participant** — a person enrolled in the event for its duration. They may optionally be linked to a real user account, but that is not required.
