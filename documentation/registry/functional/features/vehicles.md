...
Actions use CRUD shorthand — **C**reate, **R**ead, **U**pdate, **D**elete — plus *History* (a vehicle's movement history). See [Roles & Permissions](../roles-and-permissions) for the full model, and [Domain Model → Vehicle](../domain-model#vehicle-option) for the entity.
...
> **Option gating first.** All rows above assume the project has the `VEHICLE` option enabled. With the option off, the API is closed for every role — see [Roles & Permissions → Project options](../roles-and-permissions#project-options-gating).
...
- **Assigned inside movements.** A vehicle is never "used" on its own — it is assigned to participants **inside a movement**, with an optional **carpool / pool label**, and only on `REGISTERED` content (see [Movements → Business rules](./movements#3-business-rules)).
...
REST endpoints backing this feature. All project-scoped endpoints live under `/api/v1/projects/{projectId}/...` and **require the `VEHICLE` option**. See [Technical → API Reference](../../technical/api-reference).
...
| `PATCH` | `/vehicles/{id}/disable` | Soft-disable a vehicle | `VEHICLE` option + `REGISTRY_PROJECT_VEHICLE_U` |
| `PATCH` | `/vehicles/{id}/enable` | Re-enable a disabled vehicle | `VEHICLE` option + `REGISTRY_PROJECT_VEHICLE_U` |
